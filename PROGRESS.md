...
> **Next action:** Read `07-forms-basic.md` and ship `Button`/`Input`/`Textarea`/`Select`/`Checkbox`/`Radio`/`Switch` composed on the Phase 05 primitives + Phase 06 typography. Ground truth for styling tokens: `vars.color.*` + the semantic body/heading composites already in CSS.
