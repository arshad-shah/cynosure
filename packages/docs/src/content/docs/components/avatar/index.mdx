...
<div><StatusBadge status="stable" /> <BundleSizePill name="Avatar" /></div>
...
- Initials are rendered inside a Radix fallback element which exposes an `aria-label` when only a name (no derived initials) is available.
