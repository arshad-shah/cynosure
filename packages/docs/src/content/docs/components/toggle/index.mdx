---
title: Toggle
description: Two-state button that switches a single option on or off.
category: components
status: stable
since: "0.1.0"
a11y: Exposes `aria-pressed` for the on/off state, native keyboard activation (Enter/Space), and focus styles driven by design tokens.
bundleSize: "1.0 kB"
