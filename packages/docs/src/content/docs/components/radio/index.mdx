...
status: stable
since: 0.1.0
a11y: Built on Radix UI radio-group; roving tabindex managed by the group; role="radio" with aria-checked.
bundleSize: "10 kB"
tags: [form, input]
---
...
<div><StatusBadge status="stable" /> <BundleSizePill name="Radio" /></div>
...
<RadioGroup defaultValue="b" aria-label="Pick one">
...
- Built on Radix UI radio-group — manages roving tabindex so only the selected item is in the tab order.
