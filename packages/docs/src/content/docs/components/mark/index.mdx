---
title: Mark
description: Inline text highlighting for search hits, regex matches, syntax tokens, and diff hunks — a semantic `<mark>` that flows with surrounding prose and wraps cleanly across lines.
category: components
status: stable
since: "0.1.0"
...
<div><StatusBadge status="stable" /> <BundleSizePill name="Mark" /></div>
...
```tsx
import { Mark } from '@arshad-shah/cynosure-react';

<Mark>highlighted text</Mark>
```

`Mark` is a thin inline-flow primitive — reach for it instead of `Badge`/`Code` when you need to highlight a substring inside flowing text without breaking baseline alignment or vertical rhythm.
