...
`+N` badge instead of wrapping. Opening it reveals a search field and the full
option list; every item stays in the list (marked with a checkmark) and toggles
on click, so nothing becomes unreachable.
