 ...
 *  - `--lumen-typography-decoration-color` feeds `underlineClass` /
 *    `strikethroughClass` so decoration colour can diverge from text colour.
 *  - `--lumen-typography-line-clamp` feeds `lineClamp` so a single class
...
export const typographyInlineStyle = (input: TypographyStyleInput): CSSProperties | undefined => {
  ...
  if (typeof input.truncate === 'number' && input.truncate > 1) {
    out['--lumen-typography-line-clamp'] = String(input.truncate);
  }
  if (input.decorationColor !== undefined) {
    out['--lumen-typography-decoration-color'] = resolveColor(input.decorationColor);
  }
